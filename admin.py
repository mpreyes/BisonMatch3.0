...
class LustudentAdmin(admin.ModelAdmin):
    list_display = [ 'name', 'lnumber', 'emailaddress', 'gender', 'paid', 'bio','idealdate']
    ordering = ['lnumber']
    actions = ['generateMatches','markPaid', 'sendReminderEmail', 'sendResultEmail', 'delete_selected']
    ...
    def sendResultEmail(self, request, queryset):
        students = queryset
        for student in students:
            if student.paid == 1 or student.paid == 3:
                ...
                message += "To view your Bison Match results, go to http://157.230.208.45:8001/bisonmatch/matches" + student.lnumber
                ...
                subject = "Get your Bison Match Results Before 1 today!"
                message = "Hi " + student.name + ",\n"
                message += "It's not too late to view your Bison Match results!\n"
                message += "Just go to http://157.230.208.45:8001/thanks to buy your results.\n"
                message += "Once you have paid for your results, you can view them at http://157.230.208.45:8001/bisonmatch/matches" + student.lnumber
                message += "\nHappy Valentine's Day!\n\nMuch Love,\nYour Friends at Bison Match"
                send_mail(
                    subject, #subject
                    message,   #body
